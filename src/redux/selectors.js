...
export const getMatchParams = createSelector(getCurrentPath, (pathname, _) => {
  const match = deriveMatchFromPath(pathname);
...
export const getCurrentSong = createSelector(
  ...
  (songsById, songId) => {
    if (!songId) {
      console.warn(
        `The "getCurrentSong" selector has been called with no songId detected in match params`
      );
      return null;
    }
    const song = songsById[songId] || {};
